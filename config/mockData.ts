...
export const errorAddresses =
  process.env.NEXT_PUBLIC_ERROR_SYNC_ADDRESSES?.split(',') as string[];

export const syncApps = process.env.NEXT_PUBLIC_SYNC_APPS?.split(
  ','
) as AppId[];

export const errorApps = process.env.NEXT_PUBLIC_ERROR_SYNC_APPS?.split(
  ','
) as AppId[];
