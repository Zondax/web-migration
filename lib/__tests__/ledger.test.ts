...
const createMockResponse = (statusCode: number) => Buffer.concat([
...
describe('LedgerService', () => {
  describe('openApp', () => {
    ...
    it('should throw TransportStatusError when transport is undefined', async () => {
      const ledgerService = new LedgerService()
      await expect(ledgerService.openApp(undefined as any, 'Polkadot Migration'))
        .rejects
        .toThrow('TransportStatusError')
    })
  })
  ...
  describe('initializeTransport', () => {
    ...
    it('should handle transport creation failure', async () => {
      ...
      await expect(ledgerService.initializeTransport())
        .rejects
        .toThrow('Failed to create transport')
  ...
  describe('connectDevice', () => {
    ...
    it('should throw error when connection fails', async () => {
      ...
      await expect(ledgerService.connectDevice())
        .rejects
        .toThrow('Failed to establish device connection')
    })
  })
 ...
 describe('getAccountAddress', () => {
    ...
    it('should throw error when app is not open', async () => {
      ...
      const bip44Path = "m/44'/354'/0'/0/0'" // Valid Polkadot BIP44 path
      await expect(ledgerService.getAccountAddress(bip44Path, 0, true))
        .rejects
        .toThrow('App not open')
    })
  ...
  describe('signTransaction', () => {
    it('should sign transaction successfully when app is open', async () => {
      ...
      expect(mockGenericApp.signWithMetadata).toHaveBeenCalledWith(
        bip44Path,
    ...
    it('should throw error when app is not open', async () => {
      ...
      await expect(ledgerService.signTransaction(bip44Path, payloadBytes, chainId, proof1))
        .rejects
        .toThrow('28161')
    })
    ...
    it('should handle errors from signWithMetadata call', async () => {
      ...
      await expect(ledgerService.signTransaction(bip44Path, payloadBytes, chainId, proof1))
        .rejects
        .toThrow('Failed to sign transaction')
    })
