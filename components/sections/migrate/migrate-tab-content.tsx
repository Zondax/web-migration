        ...
        .map((account, index) => (
          <TableRow key={`${app.id}-${account.address}-${index}`}>
            ...
            <TableCell className="font-mono">
              {formatBalance(account.balance!, app.ticker, app.decimals)}
            </TableCell>
            <TableCell>
              <div className="flex items-center space-x-2">
                {renderStatusIcon(account)}
                {account.transaction && (
                  <TransactionDropdown transaction={account.transaction} />
                )}
              </div>
            </TableCell>
          </TableRow>
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="text-gray-600">
          Review your accounts and click Migrate to complete the process.
        </p>
      </div>
        ...
        <TableBody>
          {filteredAppsWithoutErrors.length > 0 ? (
            filteredAppsWithoutErrors.map((app) => (
              <MigrateRow key={app.id?.toString()} app={app} />
            ))
          ) : (
            <TableRow>
              <TableCell
        ...
        {!migrationResults.success && !migrationResults.total ? (
          <>
            ...
            <Button
              ...
              disabled={filteredAppsWithoutErrors.length === 0}
            >
              Migrate Accounts
            </Button>
          </>
