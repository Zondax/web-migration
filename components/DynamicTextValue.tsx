...
export const DynamicTextValue = (props: {
  ...
  const [dynamicMaxStringLength, setDynamicMaxStringLength] =
    useState<number>(50);
  ...
  const adjustDynamicMaxStringLength = useCallback(() => {
    ...
    if (textWidth > containerWidth) {
      ...
      textWidth < containerWidth - tolerance &&
      dynamicMaxStringLength < value.length
    ) {
  ...
  const observer = useMemo(
    () => new ResizeObserver(adjustDynamicMaxStringLength),
  ...
  return (
    <span ref={textRef}>
      {truncateMiddleOfString(value.toString(), dynamicMaxStringLength)}
    </span>
  );
};
